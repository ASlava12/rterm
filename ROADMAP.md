...
- [ ] **Дренировать OSC 52 у всех панелей, не только фокусной.**
  ...
  (`crates/rterm-render/src/lib.rs:~13129`) — фоновые панели копят
  запросы и внезапно перезаписывают клипборд при фокусе. Решить
  политику (применять или дропать для фоновых) и дренировать все.
  DoD: integration-тест с двумя панелями.

- [ ] **`--smoke`: изоляция от пользовательского Lua.**
  Хоистнуть проверку `--smoke` выше `PluginHost::new` /
  `load_user_lua` / `spawn_watcher` (`main.rs:~1490-1571`) — смоук не
  должен исполнять `~/.config/rterm/init.lua` (недетерминированный CI).
  Заодно: не-блокирующий join ридера в `run_smoke` (Windows/ConPTY
  может не дать EOF — CI виснет до таймаута джоба).
  DoD: смоук с сентинел-плагином в `RTERM_CONFIG_PATH` не исполняет его.
