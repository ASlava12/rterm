...
- [ ] **IME: чистить `ime_preedit` при смене фокуса.** (review-батч 3)
  Сейчас preedit чистится только на `Ime::Commit`/`Disabled`
  (`event_loop.rs:~346/356/360`). Смена таба/панели ВО ВРЕМЯ композиции
  оставляет осиротевший preedit → рисуется на новой панели, а следующий
  `Ime::Commit` уходит в неё (текст не туда). Триггер узкий (фокус-смена
  посреди композиции). Правильный фикс — центральный хук «фокус сменился»
  (сейчас ~14 сайтов `active_tab =`/`focus_path =`), а не разбросанные
  `clear()`. DoD: preedit исчезает при любой смене фокуса; commit идёт в
  панель, где шла композиция.
- [ ] **IME: клампить ширину preedit-рендера по панели.** (review-батч 3)
  Backdrop-квад + глифы preedit (`event_loop.rs` preedit-рендер,
  `TextBounds` в `lib.rs`) не клампятся к правому краю панели → длинная
  композиция у правого края сплита рисуется поверх соседней панели.
  Только визуально (нет паники). DoD: preedit обрезается/переносится по
  панели.
...
- [ ] **Restore: title применяется не к тому табу при провале spawn.** (review-батч 3)
  Цикл restore (`event_loop.rs:~96-117`) зовёт `new_tab_in`, который при
  ошибке `spawn_pane` возвращается БЕЗ push таба (`lib.rs:~5047`), затем
  безусловно ставит `entry.title` на `tabs.last_mut()` → title таба N
  затирает таб N−1. Узкий триггер (fd-исчерпание; удалённый cwd НЕ
  триггерит — фолбэк на `$HOME`). Фикс: `new_tab_in` возвращает
  bool/`Option`, title применять только если таб реально добавлен.
- [ ] **Broadcast: рассылать paste во все панели (спорно).** (review-батч 3)
  `commit_paste_now` (`input.rs:~265`) шлёт вставку только в фокусную
  панель, игнорируя `broadcast_input` (в отличие от клавиш). Возможно
  намеренно (рассылать пасту во все шеллы разом — рискованно). Решить
  дизайн: если делать — оборачивать per-pane своим `bracketed_paste()`.
