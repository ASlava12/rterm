...
rterm встраивает Lua 5.4 (через `mlua` + LuaJIT, скомпилированный внутрь бинарника). Плагины — императивный слой настройки: реагировать на события, регистрировать действия, дёргать пользовательский API.
...
Все три источника (`config.toml`, `init.lua`, `plugins/*.lua`) горячо перезагружаются по mtime. После переинициализации Lua-стейт **чистый**, и подписки регистрируются заново — поэтому хуки обычно ставят в начале файла на верхнем уровне, без условий вроде «уже подписан».
