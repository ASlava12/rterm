...
impl PsuedoCon {
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        // rterm fork: prefer PSEUDOCONSOLE_PASSTHROUGH_MODE so the
        // pty stops re-rendering child output through a virtual
        // screen buffer (which mangles non-ASCII bytes — `cat
        // picture.png` over SSH ends up as a stream of U+FFFD
        // replacement chars instead of the raw image). Passthrough
        // requires Win11 22H2+; on older Windows the flag is
        // rejected with E_INVALIDARG and we fall back to the
        // original flag set.
        let base_flags = PSEUDOCONSOLE_RESIZE_QUIRK | PSEUDOCONSOLE_WIN32_INPUT_MODE;
        ...
        if result == S_OK {
            ...
            log::warn!(
                "CreatePseudoConsole with PASSTHROUGH_MODE failed (HRESULT {}); \
                 ...
                 picture.png`) will be mangled by ConPTY's VT renderer",
                result
            );
            result = unsafe {
                (CONPTY.CreatePseudoConsole)(
